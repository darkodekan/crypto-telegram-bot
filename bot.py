...
def get_coin_price(m):
    ...
    try:
        ...
        coin_symbol = user_commands[1]
        if len(user_commands) == 3 and "/" in user_commands[2]: # /p eth 02/03 or /p eth 02/03/45
            ...
            try:
                ...
                if coin_data:
                    ...
                    bot.send_message(m.chat.id, "No data on this coin. Did you type it correctly?")

        else:
            coin_symbols = [coin_symbol] + user_commands[2:]
            formatted_prices_list = []
            for coin_symbol in coin_symbols:
                ...
                if coin_data:
                    coin_price = coin_data[coin_symbol][chat.currency_code]
                    formatted_price = currency.pretty(coin_price, chat.currency_code)
                    formatted_prices_list.append(f"{coin_symbol}: {formatted_price}")
                else:
                    formatted_prices_list.append(f"No data for {coin_symbol}. Did you type the standard code correctly?")
            formatted_prices_message = "\n".join(formatted_prices_list)
            bot.send_message(m.chat.id, formatted_prices_message)
